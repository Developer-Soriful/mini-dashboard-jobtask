...
export default function HomePage() {
  ...
  const stats = [
    { title: "Total Users", value: "2,456", color: "from-indigo-500 to-purple-500" },
    { title: "Revenue", value: "$12,402", color: "from-green-400 to-teal-500" },
    { title: "Conversion Rate", value: "4.2%", color: "from-pink-500 to-orange-400" },
  ];
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        {stats.map((stat, index) => (
          <motion.div
            key={index}
            className={`bg-gradient-to-r ${stat.color} rounded-2xl p-6 text-white shadow-xl transform transition-transform hover:scale-105 hover:shadow-2xl`}
      ...
      <motion.div
        className="rounded-3xl p-6 shadow-2xl hover:shadow-3xl transition-shadow duration-300 bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500"
        initial={{ opacity: 0, y: 30 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.5, duration: 0.6 }}
      >
        <h2 className="text-2xl font-semibold mb-4 text-white">Activity Overview</h2>
        ...
        <div className="h-64 md:h-72 p-4 bg-white/10 backdrop-blur-md rounded-2xl shadow-inner border border-white/20">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data} margin={{ top: 10, right: 20, left: 0, bottom: 0 }}>
              ...
              <YAxis yAxisId="users" orientation="left" tick={{ fill: "#fff", fontWeight: 600 }} />
              <YAxis yAxisId="revenue" orientation="right" tick={{ fill: "#fff", fontWeight: 600 }} />
              <Tooltip
                contentStyle={{
                  backgroundColor: "#fff",
                  borderRadius: "12px",
